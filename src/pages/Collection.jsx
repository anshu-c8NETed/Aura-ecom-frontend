...
export default function Collection() {
  ...
  const baseProducts = (Array.isArray(raw) && raw.length > 0) ? raw : filteredMocks

  // ── Apply sidebar filters (price, size, color) ──
  const filtered = baseProducts.filter(p => {
    // Price filter
    if (p.basePrice < priceRange[0] || p.basePrice > priceRange[1]) return false

    // Size filter — product must have at least one variant with that size in stock
    if (activeSize) {
      const hasSize = p.variants?.some(v => v.size === activeSize && v.stock > 0)
      if (!hasSize) return false
    }

    // Color filter — product must have at least one variant with that color name
    if (activeColor) {
      const hasColor = p.variants?.some(v =>
        v.color.toLowerCase() === activeColor.toLowerCase() && v.stock > 0
      )
  ...
  const products = [...filtered].sort((a, b) => {
    switch (sort) {
      ...
      case 'popular':    return (b.isFeatured ? 1 : 0) - (a.isFeatured ? 1 : 0)
      case 'newest':
      default:           return (b.isNewArrival ? 1 : 0) - (a.isNewArrival ? 1 : 0)
    }
  })
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      outline: activeColor === c.name ? '2px solid var(--ch)' : '2px solid transparent',
                      outlineOffset: '2px',
                      transition: 'outline .2s',
                      minHeight: 'unset', minWidth: 'unset',
                    }}
                    onMouseEnter={e => { if (activeColor !== c.name) e.currentTarget.style.outlineColor = 'var(--ch)' }}
                    onMouseLeave={e => { if (activeColor !== c.name) e.currentTarget.style.outlineColor = 'transparent' }}
                  />
                ))}
              </div>
            </div>
