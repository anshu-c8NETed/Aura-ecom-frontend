...
function PaymentStep({ form, setForm, errors, onBack, onNext, placing }) {
  ...
  return (
    <div className="co-step">
      ...
      {/* Card fields */}
      {form.payMethod === 'card' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '.4rem', padding: '1.4rem', background: 'var(--cr2)', border: '1px solid var(--bo)' }}>
          <Field label="Card Number" error={errors.cardNumber}>
            <input style={inputStyle(errors.cardNumber)} value={form.cardNumber} onChange={set('cardNumber')}
              onFocus={e => e.target.style.borderColor = 'var(--go)'}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <Field label="Expiry (MM/YY)" error={errors.cardExpiry}>
              <input style={inputStyle(errors.cardExpiry)} value={form.cardExpiry}
                onFocus={e => e.target.style.borderColor = 'var(--go)'}
                onBlur={e  => e.target.style.borderColor = errors.cardExpiry ? '#c0392b' : 'var(--bo)'}
                maxLength={5} placeholder="08/27"
                onChange={e => {
                  const raw = e.target.value.replace(/\D/g, '').slice(0, 4)
                  const fmt = raw.length > 2 ? `${raw.slice(0,2)}/${raw.slice(2)}` : raw
                  setForm(f => ({ ...f, cardExpiry: fmt }))
                }}
              />
            </Field>
            <Field label="CVV" error={errors.cardCvv}>
              <input type="password" style={inputStyle(errors.cardCvv)} value={form.cardCvv}
                onFocus={e => e.target.style.borderColor = 'var(--go)'}
      ...
      {/* UPI */}
      {form.payMethod === 'upi' && (
        <div style={{ padding: '1.4rem', background: 'var(--cr2)', border: '1px solid var(--bo)', marginTop: '.4rem' }}>
      ...
      {form.payMethod === 'cod' && (
        <div style={{ padding: '1rem 1.2rem', background: 'var(--cr2)', border: '1px solid var(--bo)', marginTop: '.4rem' }}>
...
export default function Checkout() {
  ...
  // Redirect empty cart (but not after confirmation)
  useEffect(() => {
    if (items.length === 0 && step < 2) navigate('/collection')
  }, [items.length, step])

  /* GSAP step transition */
  useGSAP(() => {
    gsap.from('.co-step', { opacity: 0, y: 18, duration: .5, ease: 'power2.out' })
  ...
  async function handlePlaceOrder() {
    ...
    try {
      ...
      // Optimistic confirmation for dev / network issues
  ...
  return (
    <div className="page-offset" ref={containerRef}>
      ...
      <div style={{
        ...
        {/* Left: steps */}
        <div style={{ padding: 'clamp(2rem,5vw,4rem) 5vw', borderRight: step < 2 ? '1px solid var(--bo)' : 'none' }}>
